...
import { loadTelegramUnitRules, formatTelegramMessage, selectPlaysForTelegram } from './telegram/messagingService.js';
...
// --- Constants ---
const TELEGRAM_FORMULA = process.env.TELEGRAM_FORMULA || "ml_predicted";
...
const BULK_WRITE_CHUNK_SIZE = Number.isFinite(Number(process.env.EV_BULK_CHUNK_SIZE))
  ? Math.max(1, Number(process.env.EV_BULK_CHUNK_SIZE))
  : Number.POSITIVE_INFINITY;
const BULK_WRITE_MAX_TIME_MS = Number.isFinite(Number(process.env.EV_BULK_MAX_TIME_MS))
  ? Math.max(60000, Number(process.env.EV_BULK_MAX_TIME_MS))
  : 360000;
const TELEGRAM_SCOPE_WHITELIST = new Set(['total', 'home','away']);
const TELEGRAM_SELECTION_WHITELIST = new Set(['under', 'over']); // kept for compatibility, not used when mirroring control-bot
...
//         logger.info(
//             `[EV DB] bulkWrite chunkSize=${BULK_WRITE_CHUNK_SIZE}, maxTimeMS=${BULK_WRITE_MAX_TIME_MS}, ops=${ops.length}`
//         );
...
async function runEvCalculation() {
    ...
    for (const match of upcomingMatches) {
        ...
        const { allEvResults, telegramEvResults } = await calculateAllEvsForMatch({ match, odds, homePlayerStats, awayPlayerStats }, formulas, TELEGRAM_FORMULA);
        ...
        // --- Main Bot Logic ---
        // Välj de bästa spelen efter högst EV som samtidigt matchar unit-reglerna
        const selectedPlays = selectPlaysForTelegram(
          telegramEvResults,
          {
            maxLines: TELEGRAM_MAX_LINES,
            maxPlays: TELEGRAM_MAX_PLAYS,
            scopeWhitelist: TELEGRAM_SCOPE_WHITELIST,
            selectionWhitelist: TELEGRAM_SELECTION_WHITELIST,
          },
          unitRules
        );
