 ...
 */
export const calculateEvForMatch = async (match, odds, homePlayerStats, awayPlayerStats, formulaKey = 'raz_optimal') => {
  const homeName = match.event.homeName;
  const awayName = match.event.awayName;
  const matchId = match.event.id;

  const expectedGoals = await calculateExpectedGoals(homePlayerStats, awayPlayerStats, formulaKey);
