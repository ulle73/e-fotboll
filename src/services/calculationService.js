 ...
 */
export async function calculateAllEvsForMatch(matchData, formulas, telegramFormula) {
    const { match, odds, homePlayerStats, awayPlayerStats } = matchData;
    const allEvResults = [];
    let telegramEvResults = [];

    for (const formula of formulas) {
        const evResults = await calculateEvForMatch(match, odds, homePlayerStats, awayPlayerStats, formula);
...
export function prepareBacktestingDocs(evResults, match, odds) {
    ...
    return evResults
      .map(res => {
        ...
        const bets = [];
        const overEv = Number(overBet.ev);
        const underEv = Number(underBet.ev);
        if (Number.isFinite(overEv) && overEv > 0) {
            bets.push(overBet);
        }
        if (Number.isFinite(underEv) && underEv > 0) {
            bets.push(underBet);
        }
        return bets;
    }).flat();
