...
const MAX_MATCHES_PER_PLAYER = 100; // Högst N matcher per spelare per körning.
...
const fetchTournamentMatches = async (tournamentId, urls) => {
  const matchTemplate = urls.apiRoutes?.eventMatches;
  const resultsTemplate = urls.apiRoutes?.eventResults;
  const template = matchTemplate || resultsTemplate;

  if (!template) throw new Error('eventMatches eller eventResults saknas i urls.json');
...
const processPlayer = async (player, urls, sinceTs, db, forceFullScan = false) => {
  ...
  const template = urls.apiRoutes?.rosterTournaments;
  if (!template) throw new Error('rosterTournaments saknas i urls.json');
  ...
  const totalUniqueMatches = knownIds.size + fetchedMatches;
  logger.info(
    `Spelare ${player.nickname} har nu ${totalUniqueMatches} unika matchId:n (inklusive nyhämtade).`,
  );
