...
const hasFlag = (key) => process.argv.some((arg) => arg === `--${key}` || arg.startsWith(`--${key}=`));
const normalizeArg = (value) => (typeof value === 'string' ? value.trim() : '');
...
const DEFAULT_TOLERANCE_MINUTES = Number(process.env.SETTLE_TOLERANCE_MINUTES ?? 61);
const DEFAULT_COMMAND_MAX_TIME_MS = Number(process.env.SETTLE_COMMAND_MAX_TIME_MS ?? 100000);
const COMMAND_MAX_TIME_MS = Math.max(1, DEFAULT_COMMAND_MAX_TIME_MS) * 5;
...
const eventIdRaw = argValue('eventId');
const eventIdAll = normalizeArg(eventIdRaw).toLowerCase() === 'all';
const eventIdArg = eventIdAll ? null : toNumberSafe(eventIdRaw, null);
const withSettledFilter = (filter = {}) => (backfillMode ? filter : { ...filter, settled: false });
...
const buildBetQuery = () => {
  ...
  if (Number.isFinite(eventIdArg)) {
    return withSettledFilter({ eventId: eventIdArg });
  }
  return withSettledFilter({});
};
...
const fetchCandidateEvents = async (col) => {
  ...
  if (Number.isFinite(eventIdArg)) {
    const bet = await col.findOne(withSettledFilter({ eventId: eventIdArg }));
    return bet ? [bet] : [];
  }

  const eventIds = await col.distinct(
    'eventId',
    withSettledFilter({ eventId: { $ne: null } }),
    { maxTimeMS: COMMAND_MAX_TIME_MS }
  );
  const limitedIds =
  ...
  for (const id of limitedIds) {
    const bet = await col.findOne(withSettledFilter({ eventId: id }));
    if (bet) {
      docs.push(bet);
    }
  }
  ...
  if (!eventIds.length) {
    const fallbackCursor = col.find(withSettledFilter({}));
    const fallbackBets =
...
const chooseBestMatch = (matchDocs, kickoffIso) => {
  ...
  if (bestOverall) {
    ...
    console.warn(
      `⚠️  Ingen ESB-match inom ${toleranceMinutes} minuter hittades (närmsta diff ${diffMinutes} min). Hoppar över.`
    );
  }
  return null;
};
...
const findEsbMatchForBet = async (matchesCol, bet) => {
  ...
  const candidates = await matchesCol
    .find({
      ...
      homePlayerNick: homeRegex,
      awayPlayerNick: awayRegex,
    })
    .toArray();
...
const settleBetGroup = async (db, bet, matchesCol, evBetsCol) => {
  const groupedBets = await evBetsCol
    .find(withSettledFilter({ eventId: bet.eventId }))
    .toArray();

  if (!groupedBets.length) {
    console.warn(`Inga bets hittades för event ${bet.eventId} med aktuellt filter.`);
